FROM alpine:3

RUN apk add --no-cache \
      apache2 apache2-utils apache2-webdav mod_dav_svn \
      subversion \
      php84 php84-apache2 php84-json php84-session php84-ldap php84-xml \
      wget bash \
  && sed -i 's/;extension=ldap/extension=ldap/' /etc/php84/php.ini \
  && sed -i 's/^Listen 80$/Listen 8080/' /etc/apache2/httpd.conf \
  && sed -i 's/^#\?ServerAdmin.*$/ServerAdmin info@tactical-operations.net/' /etc/apache2/httpd.conf \
  && mkdir -p /var/svn /var/log/apache2 /run/apache2 \
  && touch /etc/apache2/dav_svn.passwd  

COPY dav_svn.authz /etc/apache2/

COPY dav-svn.conf /etc/apache2/conf.d/

RUN wget -qO /tmp/svnadmin.tar.gz https://github.com/mfreiholz/iF.SVNAdmin/archive/refs/heads/master.tar.gz \
  && mkdir -p /var/www/localhost/htdocs/svnadmin/data  \
  && tar -xzf /tmp/svnadmin.tar.gz -C /var/www/localhost/htdocs/svnadmin --strip-components=1 \
  && rm -f /tmp/svnadmin.tar.gz 

RUN chown -R apache:apache /var/svn /etc/apache2 /var/log/apache2 /run/apache2 /var/www/localhost/htdocs \
  && chmod -R 777 /var/www/localhost/htdocs/svnadmin/data \
  && chmod 660 /etc/apache2/dav_svn.authz /etc/apache2/dav_svn.passwd

EXPOSE 8080
USER apache
CMD ["httpd", "-D", "FOREGROUND"]
